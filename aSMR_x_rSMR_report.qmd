...
rsmr_cmd.long <- CMD %>% pivot_longer(cols = -"BGC", names_to = "mean.rSMR", values_to = "CMD.site")
...
rsmr_asmr.long <- all_cmd %>% pivot_longer(cols = -"BGC", names_to = "rSMR", values_to = "aSMR")
fwrite(rsmr_asmr.long, file = "./outputs/aSMR_by_rSMR_long.csv")
```
...
rules <- read.csv("./inputs/aSMR_Rules_HalfStep_v13_7May2025.csv")
BGC.rain <- fread("./inputs/Coast_freq_rain_BGCs.csv") %>% filter(freq_rain %in% "y") %>% pull(bgc)
aSMRClass <- function(x) {
  for (i in seq_along(ruleSelect$CMD)) {
    if (i < length(ruleSelect$CMD) && x <= ruleSelect$CMD[i+1]) {
