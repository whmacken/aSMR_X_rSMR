...
Drought tool assignments of aSMR classes for rSMR 4 and drier sites are then compared to both expert and CMD assigned aSMR classes.
...
#allDat <- fread("./inputs/WNA_BGC_climr.csv") %>% as.data.table ## raw data from Create_BGC_climate_data.Rmd
...
moist.multiply <- temp3 %>% select(BGC, ratio) %>% mutate(multiplier = 1+ratio) %>% data.table
...
# for (i in 1:4){
#   CMD[,length(CMD)+1] <- (CMD[,length(CMD)] + 50)*1.1
# }
# for (i in 1:4) {
...
rsmr_cmd.long <- CMD %>% pivot_longer(cols = -"BGC", names_to = "rSMR", values_to = "CMD")%>%
...
aSMRdiff2 <- aSMRdiff %>% mutate(Zone = gsub("[^A-Z]", "", BGC))
...
ggplot(CMDrange, aes(rSMR4_Exp, rSMR4)) +
  ...
  ylab("CMD.total") +
  ggrepel::geom_text_repel(aes(label = BGC), size = 2, maxoverlaps = Inf)
#
