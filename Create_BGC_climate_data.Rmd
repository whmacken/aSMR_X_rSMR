...
climate_vars <- clim_vars2 %>% dplyr::select(BGC, PERIOD, CMD) %>% filter(startsWith(BGC, "SB"))

ggplot(climate_vars, aes(x = BGC, y = CMD, fill = PERIOD)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "CMD by BGC for normal period, modern periods and a recent drought year.", x = "BGC", y = "CMD")

# compare mean difference in CMD between 2001-2020 periods and 2021
drought <- climate_vars %>% 
  group_by(BGC, PERIOD) %>% 
