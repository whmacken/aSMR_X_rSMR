...
clim_vars <- climr::downscale(setDT(coords),  vars = list_vars(), return_refperiod = FALSE, obs_years = 2021, cache = TRUE)
...
climate_vars <- clim_vars_base %>% dplyr::select(BGC, PERIOD, CMD) %>% filter(startsWith(BGC, "I"))

ggplot(climate_vars, aes(x = BGC, y = CMD, fill = PERIOD)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "CMD by BGC", x = "BGC", y = "CMD")
